...
class DistFinderAlpha(Node):
    def __init__(self):
        ...
        self.setpoint = 2.0  # Setpoint deseado para la distancia al muro, ajustable
        # Parámetro del desplazamiento del vehículo
        self.desplazamiento = 0.5  # En metros
    ...
    def getRange(self, msg: LaserScan):
        ...
        theta = 5  # Diferencia angular entre 85 y 90 grados
        angle_85_deg = math.radians(-85)
        angle_90_deg = math.radians(-90)

        index_85 = int((angle_85_deg - angle_min) / angle_increment)
        index_90 = int((angle_90_deg - angle_min) / angle_increment)

        # Verificar que los índices estén dentro del rango de las mediciones
        if 0 <= index_85 < len(ranges) and 0 <= index_90 < len(ranges):
            # Obtener las mediciones, usando 12m si hay valores infinitos
            a = ranges[index_85] if not math.isinf(ranges[index_85]) else 12.0
